...
int main(){
    // test1
    auto people1 = std::make_unique<Student>("Bob", 25, "S1001");
    auto p2 = std::move(people1);
    std::cout << (people1 == nullptr) << std::endl;
    std::cout << (p2 != nullptr) << std::endl;
    p2->introduce();
    
    //test2
    std::shared_ptr<Person> people2 = std::make_shared<Student>("Bob", 25, "S1001");
    std::weak_ptr<Person> w_people2 = people2;
    if(auto ptr = w_people2.lock()){
        ptr->introduce();
    }
    people2.reset();
    std::cout << w_people2.expired() << std::endl;
    if(auto ptr = w_people2.lock()){
        ptr->introduce();
    }
